...
class ResultChart extends Model
{
    //
    protected $table = 'result_chart';

    public function chart_log_result($wiki_id){
        $result_chart = ResultChart::all()->where('wiki_id',$wiki_id)->first();
        return $result_chart;
    }

    public function chart_log_result_select($wiki_id){
        $chart_log = ChartLog::where('wiki_id',$wiki_id);
    ...
    public function chart_log_result_register($wiki_id){
        ...
        $result_flag = ResultChart::all()->where('wiki_id',$wiki_id)->first();
        if($result_flag){
    ...
    public function chart_log_delete($wiki_id){
        $result_chart = ResultChart::where('wiki_id',$wiki_id);
        $result_chart->delete();
    }
}
